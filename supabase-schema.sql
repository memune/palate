...
CREATE POLICY "Users can upload coffee images" 
  ON storage.objects FOR INSERT 
  WITH CHECK (bucket_id = 'coffee-images' AND auth.uid()::text = (storage.foldername(name))[1]);

CREATE POLICY "Users can view coffee images" 
  ON storage.objects FOR SELECT 
  USING (bucket_id = 'coffee-images');

CREATE POLICY "Users can update their coffee images" 
  ON storage.objects FOR UPDATE 
  USING (bucket_id = 'coffee-images' AND auth.uid()::text = (storage.foldername(name))[1]);
